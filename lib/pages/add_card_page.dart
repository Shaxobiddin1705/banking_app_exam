...
import 'package:banking_app_exam/services/hive_service.dart';
import 'package:banking_app_exam/services/http_service.dart';
...
class _AddCardPageState extends State<AddCardPage> {
  ...
  void _apiCreatePost(BankCard card) {
    setState(() {
      isLoading = true;
    });
  ...
  void _uploadCard() {
    if(cardNumberController.text.trim().isNotEmpty && date1Controller.text.trim().isNotEmpty && date2Controller.text.trim().isNotEmpty && cvvController.text.trim().isNotEmpty && fullNameController.text.trim().isNotEmpty) {
      _apiCreatePost(
          BankCard(cardNumberController.text.trim(), fullNameController.text.trim(), date1Controller.text.trim() + '/' + date2Controller.text.trim(), cvvController.text.trim())
      );
      // _storeNote();
            ...
            child: Container(
              decoration: const BoxDecoration(
                  image: DecorationImage(
                      image: AssetImage('assets/images/blue_color.jpg',),
                      fit: BoxFit.cover
                  )
              ),
              child: Stack(
